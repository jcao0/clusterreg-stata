*! crsivreg.ado
*! Version 2.0: Supports multiple endogenous and instrumental variables.
program define crsivreg, eclass
    version 17
    ...
    // Start Mata block
    mata {
        // --- 1. Data Setup (identical to imivreg) ---
        uniformseed(12345)
        rseed(12345)
        Y_s         = st_data(., "`depvar'")
        if ("`endog_vars'" != "") { 
    ...
    mata {
        ...
        // --- 3a. Initial Full Sample Estimations for Residuals (identical to imivreg) ---
        ...
        // --- 3b. Robust QMLE (identical to imivreg) ---
        M_ident = I(n)
        ...
        for(j=1; j<=num_resids; j++){
            ...
            SigmaHats[j] = &tempSigma
            tempCS = cholesky(tempSigma)
            CSHats[j] = &tempCS
        }

        // --- 3c. Build Full Covariance Matrix (identical to imivreg) ---
        ...
        sigLevel = .05
        Bboot = 500 
        L_signflips = 200 // Number of sign flips for CRS
        ...
        for(iCov = 1; iCov <= p_s; iCov++ ){
            ...
            for(kk = 1; kk <= l_G; kk++){
                ...
                // ================== CRS MODIFICATION START ==================
                // In CRS, we directly calculate the p-value from sign-flipping inside the loop
                for(rr = 1; rr <= Bboot; rr++){
                    ...
                    valid_idx = selectindex(btemp_raw :!= .)

                    if(rows(valid_idx) > 1){
                        btemp_valid = btemp_raw[valid_idx]
                        G_valid = rows(btemp_valid)

                        // Observed t-stat for this simulated sample
                        ...
                        if(missing(t_obs)){
                            ...
                            pValSim[rr,kk] = mean(mean(abs(t_flip) :>= abs(t_obs), 2)')
                ...
                // Need to re-run sim loop to get theta_hat and se_hat for power calc
				...
				for(rr=1; rr<=Bboot; rr++){
					...
					if(rows(valid_idx) > 1){
						...
						abootVec[rr] = mean(current_theta_C_valid)
						sbootVec[rr] = sqrt(variance(current_theta_C_valid)/G_valid)
					}
				}

				// Power calculation (same IM-like approximation is fine for choosing G*)
            ...
            // --- 3e. Final Estimation and Inference using CRS ---
            ...
            if(rows(valid_final_idx) > 1){
                ...
                SE = sd_total(theta_C_final) / sqrt(G_final_valid) // Use sd_total for CRS
                ...
                if (SE > 1e-9) {
                    ...
                    pStar = mean(mean(abs(t_flip_final) :>= abs(tVal), 2)')
                } else {
                    pStar = 1
                }
                ...
                // CI for CRS is based on the IM t-distribution approximation
    ...
    di _n as text "CRS method (IV) with learned cluster"
