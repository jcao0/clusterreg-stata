program define crsreg, eclass
        ...
        for (iCov=1; iCov<=p; iCov++) {
            ...
            for (kk=1; kk<=l_G; kk++) {
                ...
                for (rr=1; rr<=Bboot; rr++) {
                    ...
                    btemp = J(G,1,0)
                    FamaMacbethCRS(D, XmD, Yboot, D, clustering, btemp)

                    t_obs    = mean(btemp)/(sd_total(btemp)/sqrt(G))
                    sign_mat = 2 * (uniform(G, L) :> 0.5) :- 1
                    boot_means = mean(sign_mat :* btemp, 1)'
                    boot_sds = sqrt(variance(sign_mat :* btemp, 1)') / sqrt(G)
                    t_flip = boot_means :/ boot_sds

                    pVal = mean(mean(abs(t_flip) :>= abs(t_obs))')
                    pValSim[rr, kk] = pVal
                    abootVec[rr]   = mean(btemp)
                    sbootVec[rr]   = sd_total(btemp)/sqrt(G)
                }
            ...
            FamaMacbethCRS(D, XmD, Y, D, clusteringStar, btemp)
            Coef = mean(btemp)
            se   = sd_total(btemp)/sqrt(Gstar)
            tVal = Coef/se

            sign_mat = 2*(uniform(Gstar,L):>0.5):-1
            boot_means = mean(sign_mat:*btemp,1)'
            boot_sds   = sqrt(variance(sign_mat:*btemp,1)')/sqrt(Gstar)
            t_flip     = boot_means:/boot_sds

            pStar  = mean(mean(abs(t_flip):>=abs(tVal))')
    ...
    di _n as text "CRS method with learned clusters"
    ...
    di as text "{hline 85}"
    di as text %12s abbrev("`yvar'",12) _col(14) ///
       " {c |} Coefficient  Std. err.      t    P>|t|     [95% conf. interval]   Clusters"
    di as text "{hline 85}"
